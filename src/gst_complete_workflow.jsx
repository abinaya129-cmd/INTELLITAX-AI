...
const SIGNAL_CARDS = [
  ...
  { icon: Truck, color: C.freight, title: 'Goods movement', seg: 'Manufacturers & Traders', copy: 'E-way bill count and goods value show what is physically leaving the premises — freight far above declared turnover means goods moving off the books.', tag: 'E-way bill feed' },
  { icon: FileText, color: C.gstp, title: 'GSTR-1 × 2A × 3B', seg: 'Traders', copy: 'Stock-in-trade is verified on paper: purchases visible in GSTR-2A must resurface as GSTR-1 sales. Purchases with no sales expose conduit firms; invoiced sales missing from the 3B cash return expose understated turnover.', tag: 'GSTN return triangulation' },
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '0.85fr 1.15fr', gap: 50, alignItems: 'center' }} className="gap-grid">
              <div style={{ background: C.paper, borderRadius: 14, padding: '28px 26px', color: C.inkOnPaper, boxShadow: '0 20px 60px -20px rgba(0,0,0,0.5)' }}>
                ...
                {[
                  ...
                  { label: 'Goods movement (e-way value)', v: featuredDealer.impliedFreight, color: C.freight },
                  ...
                  const vals = [featuredDealer.declared, featuredDealer.impliedElec, featuredDealer.impliedFreight, featuredDealer.impliedEmp, featuredDealer.gstr1Sales].filter((v) => v != null);
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }} className="profile-grid">
                  <div style={{ background: C.paper, borderRadius: 14, padding: 26, color: C.inkOnPaper }}>
                    ...
                    {[
                      ...
                      { label: 'Goods movement (e-way value)', value: selectedDealer.impliedFreight, color: C.freight },
                      ...
                      const vals = [selectedDealer.declared, selectedDealer.impliedElec, selectedDealer.impliedFreight, selectedDealer.impliedEmp, selectedDealer.fleetImplied, selectedDealer.gstr1Sales].filter((v) => v != null);
                ...
                <div style={{ background: C.panel, border: `1px solid ${C.panelLine}`, borderRadius: 14, padding: 24, marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }} className="signal-detail-grid">
                    ...
                    {selectedDealer.segment === 'Trader' ? (
                      <div><div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: C.gstp, marginBottom: 6 }}><FileText size={13} /> GSTR TRIANGULATION</div><div style={{ fontSize: 13 }}>2A {fmtMoney(selectedDealer.purchases)} / 1A {fmtMoney(selectedDealer.gstr1Sales)}</div><div style={{ fontSize: 11.5, color: C.textFaint }}>3B returns {fmtMoney(selectedDealer.gstr3bTurnover)} · {(selectedDealer.purchaseFlowRatio * 100).toFixed(0)}% purchase flow</div></div>
                    ) : (
